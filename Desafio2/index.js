...
class Contenedor {
	...
	deleteById = async (Number) => {
		try {
			...
			if (!objetoEncontrado) {
				...
				let objeto = productos.filter((item) => item.id != Number) //corregir, pone ] en el txt
				await fs.promises.writeFile("./productos.txt", JSON.stringify(objeto, null))
	...
	deleteAll = async () => {
		try {
			await fs.promises.writeFile("./productos.txt", JSON.stringify([], null))
		} catch (err) {
			console.log(err)
		}
	}
